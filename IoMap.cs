...
namespace io {
	...
	public class IoMap : IoObject
    {
		...
		public Dictionary<string, IoObject> map = new Dictionary<string, IoObject>();
		...
		public override IoObject proto(IoState state)
		{
            ...
            pro.createSlots();
            pro.createProtos();
            pro.map = new Dictionary<string, IoObject>();
		...
		{
			// A shadow copy
			(to as IoMap).map = new Dictionary<string, IoObject>((from as IoMap).map);
		}
		...
		public IoObject lookupMap(IoObject k)
		{
			foreach (var key in map.Keys)
				if (key.ToString().Equals(k)) return map[key];
			return null;
		}

		public IoObject lookupMapValues(IoObject v)
		{
